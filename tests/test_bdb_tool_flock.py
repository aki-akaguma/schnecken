...
class TestBDBTool(unittest.TestCase):
    ...
    def setUp(self):
        ...
        ## Keep print for debugging if needed
        ## print(f"\nCreated temporary directory: {self.test_dir}")
    ...
    def _run_bdb_command(self,
        ...
        if use_temp_db:
            ...
            cmd_prefix.extend(['--db', self.db_path
                              ])  # Use explicit db path if not temp
        ...
        self.assertEqual(
            ...
            f"Command failed with unexpected exit code "
            f"{process.returncode} (expected {expected_exit_code}). "
    ...
    def test_set_and_get(self):
        ...
        key = "mykey"
        value = "myvalue"
        stdout, stderr, temp_db_message_found = self._run_bdb_command(
            ['set', key, value])
        self.assertFalse(
            temp_db_message_found)  # Ensure temp db message was present
    ...
    def test_set_overwrite_without_force(self):
        ...
        stdout, stderr, temp_db_message_found = self._run_bdb_command(
            ['set', key, value2], expected_exit_code=1)
        self.assertFalse(temp_db_message_found)
        self.assertIn(
            f"Error: Key '{key}' already exists. Use --force to overwrite.",
            stderr)
    ...
    def test_delete_non_existent_key(self):
        ...
        key = "nonexistent"
        stdout, stderr, temp_db_message_found = self._run_bdb_command(
            ['delete', key], expected_exit_code=1, use_temp_db=True)
    ...
    def test_rename(self):
        ...
        stdout, stderr, temp_db_message_found = self._run_bdb_command(
            ['get', old_key], expected_exit_code=1)
        self.assertFalse(temp_db_message_found)
    ...
    def test_rename_to_existing_key_without_force(self):
        ...
        stdout, stderr, temp_db_message_found = self._run_bdb_command(
            ['rename', key1, key2], expected_exit_code=1)
        self.assertFalse(temp_db_message_found)
        self.assertIn(
            f"Error: New key '{key2}' already exists. Use --force to overwrite.",
            stderr)
        self.assertFalse(stdout)
    ...
    def test_rename_to_existing_key_with_force(self):
        ...
        stdout, stderr, _ = self._run_bdb_command(['get', key1],
                                                  expected_exit_code=1)
        self.assertIn(f"Error: Key '{key1}' not found.",
                      stderr)  # Old key should be gone
    ...
    def test_dump_and_restore(self):
        ...
        data = {
            ...
            # shelve handles newlines internally, dump should reflect
            "keyC": "value\nwith\nnewlines",
            # Test tab in value, should still parse correctly
            "keyD": "value\twith\ttab"
        }
        ...
        self.assertFalse(temp_db_message_found)
        self.assertFalse(
            stderr)  # Dump should produce no other stderr on success

        dumped_lines = stdout.splitlines()
        dumped_lines = [line for line in dumped_lines if line.strip()
        ...
        cmd = [
            'python3', BDB_TOOL_SCRIPT, '--db', restore_db_path, 'restore',
            backup_file_path
        ]
        restore_process = subprocess.run(cmd,
                                         capture_output=True,
                                         text=True,
                                         check=False)
        #
        ...
        self.assertEqual(
            restore_process.returncode, 0, f"Restore command failed. STDOUT: "
            f"{restore_process.stdout.strip()} STDERR: {cleaned_restore_stderr_str}"
        )
        self.assertIn(
            f"Successfully restored {len(data)} key-value pairs "
            f"from '{backup_file_path}'.", cleaned_restore_stderr_str)
        self.assertFalse(restore_process.stdout)

        # Verify restored content by getting from the new DB
        for k, v in data.items():
            cmd_get = [
                'python3', BDB_TOOL_SCRIPT, '--db', restore_db_path, 'get', k
            ]
            ...
            v = v.replace('\n', ' ')
            self.assertEqual(get_process.stdout.strip(), v,
                             f"Restored value for '{k}' mismatch.")
    ...
    def test_restore_invalid_format(self):
        ...
        # Only key1, key2, another_bad_line_too_many should be restored
        self.assertIn("Successfully restored 3 key-value pairs from", stderr)
        self.assertFalse(stdout)
        ...
        stdout, stderr, _ = self._run_bdb_command(
            ['get', 'invalid_line_no_tab'], expected_exit_code=1)
        self.assertIn("Error: Key 'invalid_line_no_tab' not found.", stderr)
        #
        stdout, stderr, _ = self._run_bdb_command(['get', 'key_only'],
                                                  expected_exit_code=1)
        self.assertIn("Error: Key 'key_only' not found.", stderr)

        stdout, stderr, _ = self._run_bdb_command(
            ['get', 'another_bad_line_too_many'], expected_exit_code=0)
    ...
    def test_temp_db_cleanup(self):
        ...
        process = subprocess.run([
            'python3', BDB_TOOL_SCRIPT, '--temp-db', 'set', 'temp_key',
            'temp_value'
        ],
                                 capture_output=True,
                                 text=True,
        ...
        for line in process.stderr.splitlines():
            if "Using temporary database in:" in line:
                temp_dir_path = line.split("Using temporary database in: "
                                          )[1].split(" (base name:")[0].strip()
                break

        self.assertIsNotNone(temp_dir_path,
                             "Did not find temporary database path in stderr.")
        # After the subprocess exits, atexit should have run, so the directory should be gone.
        self.assertFalse(
            os.path.exists(temp_dir_path),
            f"Temporary directory '{temp_dir_path}' was not cleaned up.")
